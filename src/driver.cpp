...
//		ROS_INFO("direction should have changed");
//	}
//
//	else
//	{
...
int main(int argc, char **argv)
{
     ...
     //Sets the loop to publish at a rate of .5Hz
	...
	x_current_to_goal = x_goal - current_pos_x;
	ROS_INFO("x current to goal: %f", x_current_to_goal);

	y_current_to_goal = y_goal - current_pos_y;
	ROS_INFO("y current to goal: %f", y_current_to_goal);

	if (.05 <= current_to_goal || current_to_goal <= -.05)
	{
		ROS_INFO("SPINNING");
	...
	else
	{
		...
		msg.angular.z = 0;
		if((.2<= x_current_to_goal || -.2 >= x_current_to_goal) && (.2 <= y_current_to_goal ||-.2 >= y_current_to_goal))
		{
			ROS_INFO("DRIVE");
			msg.linear.x = .1;
		} 
